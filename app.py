...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:@localhost/desafio'
db = SQLAlchemy(app)
...
class Usuario(db.Model):
    ...
    created_at = db.Column(db.data.strftime("%c"))
    amount_products = 242
...
def cria_usuario():
    ...
    try:
        usuario = Usuario(name=body["name"], companyemail= body["company"])
...
def atualiza_usuario(id):
    ...
    try:
        if('name' in body):
            usuario_objeto.nome = body['name']
        if('company' in body):
            usuario_objeto.companyl = body['company']
